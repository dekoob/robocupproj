...
| `collect(Actor)` | `Actor = player(_, R)`, `R \= goalkeeper`, `possession(none,none)`, `manhattan(ActorPos, BallPos, D)`, `D =< 1` | `del(ball_at(_))`, `add(ball_at(ActorPos))`, `add(has_ball(Actor))`; `possession → (Team, R)` — lets defenders and forwards secure a loose ball without goalkeeper involvement |
